...
This starter kit gives you everything you need—and lets you choose between two alternative challenges:
...
Create or select your Foundry Project
...
Local development: Build and test your custom agent solution locally with the OSS Microsoft Agent Framework.
Cloud-based development: Use Microsoft Foundry to orchestrate agents in the cloud using the UI or SDK.
...
Baseline Flow
The learner provides the topics they want to study.
A Learning Path Curator suggests relevant content, based on the learner's goals and role.
A Study Plan Generator converts that content into a practical study schedule, accounting for the learner's workload.
An Engagement Agent keeps the learner on track, adapting reminders to work patterns and focus windows.
Once the learner is ready, an Assessment Agent evaluates readiness using grounded, cited questions.
If the assessment passes, the system recommends the next certification or advancement step. Otherwise, it loops back into the preparation workflow.
A Manager Insights Agent provides visibility into team progress, risk areas, and completion patterns.
💡TIP
...
A learner asks for help preparing for a certification.
Foundry IQ retrieves grounded learning materials and certification guidance from the approved organisational knowledge base.
Fabric IQ interprets structured data such as required skills by role, recommended study hours, and prior synthetic team study outcomes.
Work IQ helps identify realistic study windows for each team member based on their meeting load and focus patterns.
The Study Plan Generator produces a practical, capacity-aware schedule for the team.
The Engagement Agent uses work context to keep team members on track without disrupting peak work periods.
The Assessment Agent creates grounded questions and evaluates each team member's readiness.
The Manager Insights Agent surfaces team-level progress, risk areas, and readiness summaries.
The system either recommends the next certification step for ready employees or loops back into the study preparation workflow for those who need more time.
...
Challenge B - Role-Play-Game System Scenario
...
The player enters a prompt:

"I enter the ruined chapel and search for clues."

The Game Master Agent interprets the action, identifies that this is a search/investigation scene, and checks current world state.

The Game Master asks the character agents to participate:
...
The Game Master uses code interpreter to roll the required checks.

The Game Master queries Foundry IQ for world lore about the chapel, the Moonlit Gate, and the Starwell relic.

The Game Master optionally uses web search for general public-domain inspiration, such as medieval chapel architecture or folklore motifs.

The Game Master updates the shared campaign state.

The Game Master returns the final response:
...
Memory in Foundry Agent Service: store and retrieve long-running campaign state, persist character sheets, inventory, health, conditions, quest progress, faction reputation, and world flags.
...
The player asks about the Moonlit Gate.
The Game Master queries Foundry IQ for "Moonlit Gate", "Starwell Relic", and current location.
Foundry IQ returns relevant lore: the gate opens only when three party members speak conflicting truths.
The Game Master asks the character agents to react in character.
The Game Master narrates the scene and offers the player choices.
...
World summary: one page describing the realm and its central conflict.
Starting location: one village, tavern, forest, ruin, or dungeon entrance.
Main quest: one objective with three clues and one twist.
Party profiles: one short profile for each AI character agent.
Rival profile: one recurring character with a secret motivation.
Faction profile: one ally faction and one opposing faction.
Artifact profile: one magical item tied to the quest.
Monster profile: one encounter with strengths and weaknesses.
Homebrew rules: simple checks, combat, health, and inventory rules.
Session state template: JSON or Markdown template for tracking what changed.
...
Retrieve static lore for the current location, faction, artifact, or NPC.
Retrieve character backstories for any character agents involved in the scene.
Retrieve current session notes or state summaries.
Combine retrieved context with the player's action.
Ask character agents to respond in character.
Narrate the outcome and update session state.
...
Entry agent hosted in Foundry Agent Service
...
Critic / Verifier — Add a validation layer before a final answer is returned
Self-reflection and iteration — Allow review and refinement when confidence is low
Role-based specialisation — Give agents clear responsibilities to reduce overlap
...
Ask in the Discord #agentsleague channel
Open a new issue using the repository template
